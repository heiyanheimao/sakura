...
class Admin extends Base
{
        ...
        if (self::checkRequest($request)) {
            ...
            $validate->rule([
                ...
                'admin_repassword' => function ($v, $all) {
                    if ($v != $all['admin_password']) return '两次密码不一致';
                    return true;
                },
                'admin_name' => function ($v) {
                    if (mb_strlen($v) <=1 || mb_strlen($v) > 12) return '姓名在2到12个字符';
                    return true;
                },
                'admin_type' => ValidateRule::regex('/^[12]$/', '请输入一个合法的类型'),
                'level' => function ($v, $all) {
                    if ($all['admin_type'] == 1 && null == $v) return '请给一个权限';
                    if ($all['admin_type'] == 2 && null != $v) return '请勿给一个权限';
                    if ($all['admin_type'] == 1) {
                        foreach ($v as $k =>$vv) {
                            if (!in_array($k, [1,2,3,4,5,6,9])) return '请输入一个合法的权限';
                        }
                    }
                    return true;
                },
    ...
    public function changeStatus(Request $request)
    {
        if (self::checkRequest($request)) {
            ...
            $validate->rule([
                'state' => function ($v) {
                    if ($v == '启用' || $v == '禁用' || $v == '删除') return true;
                    return '请给一个合法的数据';
                },
                'admin_id' => function ($v) {
                    if (!is_numeric($v) || $v < 0) return '请给一个合法的数据';
                    return true;
                }
            ]);
            if (!$validate->check($input)) {
    ...
    public function delAll(Request $request)
    {
        if (self::checkRequest($request)) {
            ...
            $validate->rule([
                'admin_id' => function ($v) {
                    if (!is_array($v)) return '请给一个合法的数据';
                    return true;
                },
            ]);
            if (!$validate->check($input)) {
